...
public class ProductDto {
    ...
    public UploadStatusData uploadByFile(MultipartFile file) throws ApiException{
        ConversionResult<Product> conversionResult = productFlow.convert(file);

        List<Product> productList = conversionResult.getValidRows();
        List<String> errors = conversionResult.getErrors();
        Integer totalRows = productList.size() + errors.size();

        productFlow.upload(productList,errors);

        return ProductUtil.convert(totalRows,errors);
    }
}
